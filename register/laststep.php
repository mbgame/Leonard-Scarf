...
/*    echo $hash;
   ...
   if($buytype=="omdebuy")
    {
     $search = "SELECT email, hash, active FROM omde WHERE email='".$email."' AND hash='".$hash."' AND active='0'"; 
    }
    if($buytype=="khordebuy")
    {
     $search = "SELECT email, hash, active FROM khorde WHERE email='".$email."' AND hash='".$hash."' AND active='0'"; 
    }
...
$result = mysqli_query($mysqli,$search);
   ...
   if (!$result) 
   {
    echo "DB Error, اشکال در ارتباط با سرور\n";
     echo '</br>';  
     echo '<span><a href="http://www.leonardscarf.com/register/register.php" class="myButton">ثبت نام دوباره</a><span>';
...
// echo $row['hash'];
  ...
  if($activecode==$hash)
  {
    if($buytype=="omdebuy")
    {
     $sql = "UPDATE omde SET active='1' WHERE email='".$email."' AND hash='".$hash."' AND active='0'";
    } 
    if($buytype=="khordebuy")
    {
     $sql = "UPDATE khorde SET active='1' WHERE email='".$email."' AND hash='".$hash."' AND active='0'";
    }   
...
$mail->AltBody .= 'your password : '. $password .'\r\n';
if(!$mail->send()) {
